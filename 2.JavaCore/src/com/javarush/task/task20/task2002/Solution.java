package com.javarush.task.task20.task2002;

import java.io.*;

import java.util.ArrayList;
import java.util.Date;
import java.util.List;


/* 
Читаем и пишем в файл: JavaRush
*/
public class Solution {
    public static void main(String[] args) {
        //you can find your_file_name.tmp in your TMP directory or fix outputStream/inputStream according to your real file location
        //вы можете найти your_file_name.tmp в папке TMP или исправьте outputStream/inputStream в соответствии с путем к вашему реальному файлу
        try {
            File your_file_name = File.createTempFile("your_file_name", ".txt",new File("L:\\"));
            OutputStream outputStream = new FileOutputStream(your_file_name);
            InputStream inputStream = new FileInputStream(your_file_name);

            JavaRush javaRush = new JavaRush();
            //initialize users field for the javaRush object here - инициализируйте поле users для объекта javaRush тут
            User newUser = new User();
            newUser.setFirstName("Ivan");
            newUser.setLastName("Ivanov");
            newUser.setMale(true);
            newUser.setBirthDate(new Date());
            newUser.setCountry(User.Country.UKRAINE);
            javaRush.users.add(newUser);
            javaRush.save(outputStream);
            outputStream.flush();

            JavaRush loadedObject = new JavaRush();
            loadedObject.load(inputStream);
            //check here that javaRush object equals to loadedObject object - проверьте тут, что javaRush и loadedObject равны
            System.out.println(javaRush.equals(loadedObject));
            outputStream.close();
            inputStream.close();

        } catch (IOException e) {
            //e.printStackTrace();
            System.out.println("Oops, something wrong with my file");
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Oops, something wrong with save/load method");
        }
    }

    public static class JavaRush {
        public List<User> users = new ArrayList<>();

        public void save(OutputStream outputStream) throws Exception {
            //implement this method - реализуйте этот метод
            //SimpleDateFormat f = new SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH);
            PrintWriter printWriter = new PrintWriter(outputStream);
            Boolean isUsersEmpty = this.users.isEmpty();
            printWriter.println(isUsersEmpty.toString());
            if(!isUsersEmpty){
                for (User u : this.users){
                    printWriter.println("nextuser");
                    printWriter.println(u.getFirstName());
                    printWriter.println(u.getLastName());
                    printWriter.println(u.getBirthDate().getTime());
                    printWriter.println(u.getCountry());
                    printWriter.println(u.isMale());

                }
            }
            printWriter.close();
        }

        public void load(InputStream inputStream) throws Exception {
            //implement this method - реализуйте этот метод
            BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream));
            String isUserEmpty = reader.readLine();
            if(isUserEmpty.equals("false")){
                String userReader;
                while((userReader = reader.readLine()) != null){
                    User tempUser = new User();
                    tempUser.setFirstName(reader.readLine());
                    tempUser.setLastName(reader.readLine());
                    String date = reader.readLine();
                    if(!date.equals("null"))
                        tempUser.setBirthDate(new Date(Long.parseLong(date)));
                    else
                        tempUser.setBirthDate(null);
                    String country = reader.readLine();
                    if(country.equals("UKRAINE")){
                        tempUser.setCountry(User.Country.UKRAINE);
                    }else if(country.equals("RUSSIA")){
                        tempUser.setCountry(User.Country.RUSSIA);
                    }else{
                        tempUser.setCountry(User.Country.OTHER);
                    }
                    String male = reader.readLine();
                    tempUser.setMale(male.equals("true"));
                    this.users.add(tempUser);
                }
            }
            reader.close();
        }

        @Override
        public boolean equals(Object o) {
            if (this == o) return true;
            if (o == null || getClass() != o.getClass()) return false;

            JavaRush javaRush = (JavaRush) o;

            return users != null ? users.equals(javaRush.users) : javaRush.users == null;

        }

        @Override
        public int hashCode() {
            return users != null ? users.hashCode() : 0;
        }
    }
}
